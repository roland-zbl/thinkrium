...
const DroppableNavItem = ({ item, isActive, onClick, expanded }) => {
    ...
    const style = {
        ...
        backgroundColor: isOver ? (document.documentElement.classList.contains('dark') ? 'rgba(255, 255, 255, 0.1)' : 'rgba(0, 0, 0, 0.05)') : undefined
    };
    ...
    return (
        <button
            ...
            className={cn(
                ...
                isOver ? "text-primary bg-primary/10 pl-2 rounded" : "text-muted-foreground hover:text-primary"
            )}
            onClick={() => {
...
export const Sidebar: React.FC = () => {
    ...
    return (
        ...
        >
            ...
            <div className="flex-1 py-4 flex flex-col gap-2">
                ...
                {sidebarExpanded && (
                    <div className="px-4 py-2 flex flex-col gap-1 overflow-auto">
                        <span className="text-xs uppercase text-muted-foreground font-bold mb-2 tracking-wider">近期專案</span>
                        {mockProjects.filter(p => p.status === 'active').slice(0, 5).map(proj => (
            ...
            {/* 底部工具 */}
            <div className="py-4 border-t flex flex-col gap-2" style={{ borderColor: tokens.colors.bgSubtle }}>
                <button
