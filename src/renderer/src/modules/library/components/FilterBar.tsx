    ...
    return (
        ...
        >
            <div className="flex items-center gap-2 text-muted-foreground mr-2">
                <Filter size={16} />
                <span className="text-xs font-bold uppercase tracking-widest text-muted-foreground">過濾</span>
            </div>
                    ...
                    <DropdownMenu key={cfg.key}>
                        <DropdownMenuTrigger asChild>
                            <button className="flex items-center gap-2 text-[13px] text-muted-foreground hover:text-foreground transition-colors focus:outline-none font-medium group">
                                <span className="opacity-60 group-hover:opacity-80 transition-opacity">{cfg.label}:</span>
                                <span className={cn(filters[cfg.key] !== '全部' && "text-primary")}>
                                    {filters[cfg.key]}
                                </span>
                                <ChevronDown size={14} className="opacity-50 text-muted-foreground group-hover:text-foreground" />
                            </button>
                        </DropdownMenuTrigger>
                            ...
                            {cfg.options.map((opt) => (
                                <DropdownMenuItem
                                    ...
                                    className={cn(
                                        ...
                                        filters[cfg.key] === opt && "text-primary font-medium bg-primary/10 hover:bg-primary/20 focus:bg-primary/20 focus:text-primary"
                                    )}
                                >
                                    {opt}
                                </DropdownMenuItem>
