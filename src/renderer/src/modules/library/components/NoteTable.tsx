...
export const NoteTable: React.FC = () => {
    ...
    const filteredNotes = useMemo(() => {
        return notes.filter(n => {
            ...
            // Tag 過濾
            const matchTag = filters.tag === '全部'
                ? true
                : (n.tags || []).includes(filters.tag);
                ...
                {filteredNotes.length > 0 ? (
                            ...
                            return (
                                <div
                                    ...
                                    className={cn(
                                        ...
                                        isActive ? "bg-primary/10 text-primary font-medium" : "text-foreground hover:bg-black/5 dark:hover:bg-white/[0.04] hover:text-foreground"
                                    )}
                                    style={{
                                ...
                                >
                                    ...
                                    <div className="flex-1 px-2 font-medium truncate flex items-center gap-2 text-base">
                                        <FileText size={18} className={isActive ? "text-primary" : "text-muted-foreground group-hover:text-primary/60"} />
                                        {note.title}
                                    </div>
                                    <div className="w-24 px-2 text-center capitalize">
                                        <span className="bg-black/5 dark:bg-white/5 px-2.5 py-1 rounded text-xs text-muted-foreground">{note.type}</span>
                                    </div>
                    ...
                    <div className="h-full flex items-center justify-center">
                        <EmptyState
                            ...
                            action={{
                                label: "建立新筆記",
                                onClick: () => addTab({ id: 'new-note', type: 'editor', title: '未命名筆記', data: {} })
                            }}
                        />
                    </div>
                )}
