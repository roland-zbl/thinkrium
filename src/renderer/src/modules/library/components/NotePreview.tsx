...
export const NotePreview: React.FC = () => {
    ...
    return (
        ...
        >
            {/* 工具欄 */}
            <div className="h-12 border-b flex items-center justify-between px-6 shrink-0" style={{ borderColor: tokens.colors.bgSubtle }}>
                ...
                <div className="flex gap-2">
                    <button className="p-2 hover:text-primary text-muted-foreground transition-colors tooltip" title="分享">
                        <Share2 size={16} />
                    </button>
                    <button
                        onClick={() => addTab({
            ...
            <div className="flex-1 overflow-auto p-8 space-y-6">
                ...
                <div className="flex items-center gap-4 text-xs text-muted-foreground pb-6 border-b" style={{ borderColor: tokens.colors.bgSubtle }}>
                    ...
                    <div className="flex items-center gap-1">
                        <span>類型:</span>
                        <span className="bg-white/5 px-2 py-0.5 rounded text-muted-foreground capitalize">{note.type}</span>
                    </div>
                </div>

                <div className="py-6 space-y-4">
                    <h4 className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider">關聯專案</h4>
                    <div className="flex flex-wrap gap-2">
                        {note.projects.length > 0 ? note.projects.map(p => (
                            <div key={p} className="flex items-center gap-2 bg-warning/5 border border-warning/10 text-warning px-3 py-1.5 rounded-full text-xs font-medium cursor-pointer hover:bg-warning/10 transition-colors">
                                <FolderPlus size={12} />
                                {p}
                            </div>
                        )) : (
                ...
                <div className="space-y-4">
                    <h4 className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider">內容摘要</h4>
