...
export const ProjectSelectorDialog: React.FC<Props> = ({ isOpen, onClose, onSelect, title = '加入專案' }) => {
    ...
    const filteredProjects = mockProjects.filter(p =>
        p.title.toLowerCase().includes(search.toLowerCase()) && p.status === 'active'
    );
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-start justify-center pt-20 bg-black/40 backdrop-blur-sm" onClick={onClose}>
            ...
            >
                <div className="p-3 border-b flex items-center gap-2" style={{ borderColor: tokens.colors.bgSubtle }}>
                    ...
                    <div className="flex gap-1">
                        <kbd className="px-1.5 py-0.5 rounded bg-black/5 dark:bg-white/10 text-[10px] text-muted-foreground font-mono">↑</kbd>
                        <kbd className="px-1.5 py-0.5 rounded bg-black/5 dark:bg-white/10 text-[10px] text-muted-foreground font-mono">↓</kbd>
                        <kbd className="px-1.5 py-0.5 rounded bg-black/5 dark:bg-white/10 text-[10px] text-muted-foreground font-mono">↵</kbd>
                    </div>
                </div>
                ...
                <div className="max-h-[300px] overflow-y-auto p-1">
                    {filteredProjects.length === 0 ? (
                        ...
                        filteredProjects.map((project, index) => (
                            ...
                            >
                                <Folder size={16} className={cn(index === selectedIndex ? "text-white" : "text-primary")} />
                                <span className="flex-1">{project.title}</span>
                                {index === selectedIndex && (
                                    <span className="text-xs opacity-70">Enter 選擇</span>
                                )}
                            </button>
                        ))
                    )}
                ...
                <div className="p-2 border-t bg-black/5 dark:bg-black/20" style={{ borderColor: tokens.colors.bgSubtle }}>
