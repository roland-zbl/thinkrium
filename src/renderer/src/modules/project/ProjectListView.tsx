                    ...
                    {statuses.map((status) => {
                        ...
                        return (
                            <section key={status.id} className="space-y-4">
                                <div className="flex items-center gap-3 border-b pb-3" style={{ borderColor: tokens.colors.bgSubtle }}>
                                    <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: status.color }} />
                                    <h2 className="text-sm font-bold uppercase tracking-widest text-muted-foreground">{status.label}</h2>
                                    <span className="text-xs text-muted-foreground ml-auto">{statusProjects.length} 個專案</span>
                                </div>
                                        ...
                                        >
                                            ...
                                            <div className="grid grid-cols-2 gap-4 mt-6">
                                                <div className="flex flex-col gap-1.5">
                                                    <span className="text-xs text-muted-foreground uppercase font-bold tracking-tight">素材</span>
                                                    <span className="text-xl font-light text-foreground">{proj.materialCount}</span>
                                                </div>
                                                <div className="flex flex-col gap-1.5">
                                                    <span className="text-xs text-muted-foreground uppercase font-bold tracking-tight">產出</span>
                                                    <span className="text-xl font-light text-foreground">{proj.deliverableCount}</span>
                                                </div>
                                            </div>
