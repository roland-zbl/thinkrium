                ...
                <div className="max-w-6xl mx-auto">
                    <div className="flex items-center justify-between mb-6">
                        ...
                        <div className="flex gap-2">
                            <button className="p-2 hover:bg-black/5 dark:hover:bg-white/5 rounded-lg text-muted-foreground hover:text-foreground transition-colors"><Settings2 size={18} /></button>
                            <button className="p-2 hover:bg-black/5 dark:hover:bg-white/5 rounded-lg text-muted-foreground hover:text-foreground transition-colors"><MoreVertical size={18} /></button>
                        </div>
                    </div>
                    ...
                    <div className="flex items-end justify-between">
                        <div className="space-y-2">
                            <div className="flex items-center gap-3">
                                ...
                                <div
                                    className={cn(
                                        ...
                                        project.status === 'active' ? "text-primary border-primary/20 bg-primary/5" : "text-muted-foreground border-border"
                                    )}
                                >
                                    {project.status}
                                </div>
                            ...
                            <div className="flex items-center gap-4 text-muted-foreground text-xs">
                                <span className="flex items-center gap-1.5"><Clock size={14} /> 建立於 2026-01-01</span>
                                {project.targetDate && (
                                    <span className="flex items-center gap-1.5 text-warning/60"><History size={14} /> 目標日期: {project.targetDate}</span>
                                )}
                            </div>
                        </div>
                    </div>
                ...
                <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <section className="space-y-4">
                            ...
                            <div className="bg-black/[0.02] dark:bg-white/[0.02] border border-border rounded-xl overflow-hidden">
                                {relatedNotes.map(note => (
                                    <div key={note.id} className="flex items-center justify-between p-4 border-b border-border hover:bg-black/[0.03] dark:hover:bg-white/[0.03] transition-colors cursor-pointer group">
                                        <div className="flex items-center gap-3">
                                            <FileText size={16} className="text-muted-foreground group-hover:text-primary transition-colors" />
                                            <span className="text-sm text-foreground/80 group-hover:text-foreground transition-colors">{note.title}</span>
                                        </div>
                    ...
                    <div className="space-y-8">
                        <section className="bg-black/[0.03] dark:bg-white/[0.03] border border-border p-6 rounded-xl space-y-4">
                            <h2 className="text-xs uppercase font-bold text-muted-foreground tracking-wider">策劃筆記</h2>
                        ...
                        <section className="bg-black/[0.03] dark:bg-white/[0.03] border border-border p-6 rounded-xl space-y-4">
                            <h2 className="text-xs uppercase font-bold text-muted-foreground tracking-wider">專案狀態</h2>
                            <div className="space-y-2">
                                {(['active', 'pending', 'completed'] as const).map(s => (
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            project.status === s ? "bg-primary text-white font-bold" : "text-muted-foreground hover:bg-black/5 dark:hover:bg-white/5 hover:text-foreground"
                                        )}
                                    >
                                        {s.toUpperCase()}
                                    </button>
