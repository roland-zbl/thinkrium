    ...
    return (
        ...
        >
            <div className="p-4 flex items-center justify-between border-b border-border">
                <span className="font-bold text-xs uppercase text-muted-foreground tracking-wider">訂閱源</span>
            ...
            <div className="flex-1 overflow-auto py-2">
                <button
                    ...
                    className={cn(
                        ...
                        activeSubscriptionId === null ? "bg-primary/10 text-primary font-medium" : "text-foreground/80 hover:bg-black/5 dark:hover:bg-white/5 hover:text-foreground"
                    )}
                >
                    <ListFilter size={18} />
                    <span>全部項目</span>
                ...
                {categories.map(cat => (
                    <div key={cat} className="mt-6">
                        <div className="px-4 py-2 text-xs font-bold text-muted-foreground uppercase tracking-widest">{cat}</div>
                        {subscriptions.filter(s => s.category === cat).map(sub => (
                            <button
                                key={sub.id}
                                onClick={() => setActiveSubscription(sub.id)}
                                className={cn(
                                    "w-full flex items-center justify-between px-4 py-2 text-sm transition-colors group",
                                    activeSubscriptionId === sub.id ? "bg-primary/10 text-primary font-medium border-r-2 border-primary" : "text-foreground/80 hover:bg-black/5 dark:hover:bg-white/5 hover:text-foreground"
                                )}
                            >
