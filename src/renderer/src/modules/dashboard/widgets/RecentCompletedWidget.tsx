            ...
            <div className="flex-1 space-y-2 overflow-auto no-scrollbar">
                {completedProjects.length > 0 ? (
                    completedProjects.map(proj => (
                        <div key={proj.id} className="flex items-center justify-between p-2 rounded text-muted-foreground group hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                            <span className="text-sm truncate text-foreground">{proj.title}</span>
                            <span className="text-xs whitespace-nowrap text-muted-foreground/80">{proj.targetDate}</span>
                        </div>
                    ))
                ) : (
